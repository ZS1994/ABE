...
function provinceLocation(option)
{
	...
	if(option.sheng_val == ""){
		...
		option.sheng_val == "-1";
		gpm.def_province = ["--请选择省份--", -1];
		gpm.initProvince(document.getElementById(option.sheng));
	}else{
		gpm.def_provinceoption=option.sheng_val;
		gpm.def_city1option=option.shi_val;
		gpm.initProvince(document.getElementById(option.sheng));
	}
}
...
function initLocation(option)
{
	...
	if(option.sheng_val == ""){
		...
		gpm.def_provinceoption=option.sheng_val;
		gpm.def_city1option=option.shi_val;
		gpm.def_city2option=option.xian_val;
		gpm.def_city3option=option.xiang_val;
		gpm.initProvince(document.getElementById(option.sheng));
		gpm.initCity1(document.getElementById(option.shi), gpm.def_provincekey);
		gpm.initCity2(document.getElementById(option.xian),  gpm.def_provincekey,  gpm.def_city1key);
		gpm.initCity3(document.getElementById(option.xiang), gpm.def_provincekey,  gpm.def_city1key,gpm.def_city2key);
