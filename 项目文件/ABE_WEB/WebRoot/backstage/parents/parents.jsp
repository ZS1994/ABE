			...
			<tbody>
			    ...
			    <tr>
					...
					<td width="5%" align="center">
						<a class="easyui-linkbutton" onclick="update('${p.ipId }','${p.ipName}','${p.ipSex }',
						'<fmt:formatDate value="${p.ipBirthday}" pattern="yyyy-MM-dd" />','${p.ipPhone  }',
						'${p.ipAddress}')">修改</a>
						<a class="easyui-linkbutton" href="<%=path %>/web/parents!delete?id=${p.ipId}&token=${token}" onclick="return confirm('确定删除吗?')">删除</a>
					</td>
			    </tr>
			    </c:forEach>
			</tbody>
